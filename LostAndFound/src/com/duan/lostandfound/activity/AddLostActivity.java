...
public class AddLostActivity extends Activity implements OnClickListener {
	...
	public void getAddLostInfo() {
		...
		if (!TextUtils.isEmpty(title) && !TextUtils.isEmpty(content)) {
			if (telephoneFormat(telephone) == 1) {
				...
				requestParams.setRequestType(RequestParam.ADDLOST);
				// 传输过去的数据
				try {
				...
				new AddLostMyAsyncTask().execute(requestParams);
			}
	 ...
	 * 增加失物招领的异步通信任务
	 */
	public class AddLostMyAsyncTask extends
		...
		protected Integer doInBackground(RequestParam... params) {
			...
			System.out
					.println("---Duan:AddLostMyAsyncTask.doInBackground.requestParams--->"
							+ requestParams.getJSON());
			if (!HttpClient.isConnect(AddLostActivity.this)) {
				System.out
						.println("---Duan:AddLostMyAsyncTask.doInBackground.!HttpClient.isConnect(getActivity())--->");
				return -1; // 表示请求失败
			}
			String response = Request.request(requestParams.getJSON());
			System.out
					.println("----Duan:AddLostMyAsyncTask.doInBackground.response--->"
							+ response);
			System.out
					.println("----Duan:AddLostMyAsyncTask.doInBackground.response2--->"
			...
			System.out
					.println("----Duan:AddLostMyAsyncTask.doInBackground.alaysisResponse.getCourseVideoInfo() --->"
		...
		protected void onPostExecute(Integer result) {
			...
			switch (result) {
			case 0:
				Toast.makeText(AddLostActivity.this, "添加成功！",
						Toast.LENGTH_SHORT).show();

				break;
			case -1:
				Toast.makeText(AddLostActivity.this, "添加失败！",
						Toast.LENGTH_SHORT).show();
				break;
			}
